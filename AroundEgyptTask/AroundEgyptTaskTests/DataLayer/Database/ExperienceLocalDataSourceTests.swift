...
final class ExperienceLocalDataSourceTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        sut = ExperienceLocalDataSource(modelContainer: modelContainer)
    }
    ...
    func test_getRecommendedExperiences_allRecommended_returnsAll() async throws {
        ...
        try await sut.saveExperiences(items: items)
    ...
    func test_getRecommendedExperiences_afterUpdatingItems_reflectsChanges() async throws {
        let items = [
            makeExperience(id: "1", isRecommended: true, order: 0)
        ]
        try await sut.saveExperiences(items: items)

        var results = try await sut.getRecommendedExperiences()
        XCTAssertEqual(results.count, 1)
        XCTAssertEqual(results[0].id, "1")

        // Update item to no longer be recommended
        let updated = makeExperience(id: "1", isRecommended: false)
        try await sut.saveExperiences(items: [updated])
...
/////
   func test_getExperienceDetails_existingId_returnsCorrectExperience() async throws {
    ...
    try await sut.saveExperiences(items: [experience])
...
func test_getExperienceDetails_emptyDatabase_throwsNotFound() async throws {
    ...
    func test_saveExperiences_updatesExistingItems() async throws {
        ...
        try await sut.saveExperiences(items: [original])
    ...
    func test_saveExperiences_deletesRemovedItems() async throws {
        ...
        try await sut.saveExperiences(items: initial)
    ...
    func test_searchExperiences_returnsMatchingItem() async throws {
        ...
        try await sut.saveExperiences(items: [item])
    ...
    func test_likeExperience_invalidId_throwsUpdateFailedWithNotFound() async throws {
        do {
            _ = try await sut.likeExperience(id: "invalid")
